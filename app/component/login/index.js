...
const LoginComponent = (props) => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setDeviceInfo({
        ...
        mobno:
          values[3] != "unknown" && values[3] != "" ? values[3] : "9876543210",
      });
    });
  }, []);
  ...
  useEffect(() => {
    if (
      ...
      Object.keys(props?.registrationData).length > 0
    ) {
  ...
  return (
    <View style={styles.container}>
      ...
      {state.error != null && <Text style={styles.error}>{state.error}</Text>}
      <TouchableHighlight
